'use strict';

var path = require('path');
var fs = require('fs');
var assert = require('assert');
var BookmarkDedupe = require('../index.js');

describe('BookmarkDedupe', function () {

	var expectedParse, b, testOut = '__test.deduped.html';

	after(function () {
		fs.unlinkSync(testOut);
	});

	beforeEach(function () {

		b = new BookmarkDedupe(path.join(__dirname, 'resources/test.html'), testOut);

		expectedParse = {};

		expectedParse[b.defaultH1()] = {
			contents: {
				'cheeriojs/cheerio': {
					url: 'https://github.com/cheeriojs/cheerio',
					add_date: 1443286719,
					icon: 'data:image/png;base64,'
				},
				'cheeriojs/cheerio (1)': {
					url: 'https://github.com/cheeriojs/cheerio',
					add_date: 1443357605,
					icon: 'data:image/png;base64,'
				},
				'Bookmarks Bar': {
					contents: {
						'a': {
							contents: {
								'bahamas10/node-netscape-bookmarks': {
									url: 'https://github.com/bahamas10/node-netscape-bookmarks',
									add_date: 1443286706,
									icon: 'data:image/png;base64,'
								},
								'bahamas10/node-netscape-bookmarks (1)': {
									url: 'https://github.com/bahamas10/node-netscape-bookmarks',
									add_date: 1443357570,
									icon: 'data:image/png;base64,'
								},
								'b': {
									contents: {
										'bahamas10/node-netscape-bookmarks': {
											url: 'https://github.com/bahamas10/node-netscape-bookmarks',
											add_date: 1443357649,
											icon: 'data:image/png;base64,'
										},
										'c': {
											contents: {
												'cheeriojs/cheerio': {
													url: 'https://github.com/cheeriojs/cheerio',
													add_date: 1443357598,
													icon: 'data:image/png;base64,'
												},
											}
										},
										'd': {
											contents: {}
										}
									}
								}
							}
						}
					}
				},
				'e': {
					contents: {}
				}
			}
		};
	});

	it('parses bookmark export', function () {

		assert.deepEqual(b.parse(), expectedParse);
	});

	it('deduplicates correctly', function () {
		const topNode = b.defaultH1();
		delete expectedParse[topNode].contents['Bookmarks Bar'].contents.a.contents.b.contents['bahamas10/node-netscape-bookmarks'];
		delete expectedParse[topNode].contents['Bookmarks Bar'].contents.a.contents['bahamas10/node-netscape-bookmarks (1)'];
		delete expectedParse[topNode].contents['cheeriojs/cheerio'];
		delete expectedParse[topNode].contents['cheeriojs/cheerio (1)'];

		assert.deepEqual(b.dedupe(), expectedParse);
	});

	it('deduplicates and saves to file correctly', function () {

		b.save(b.dedupe());

		assert.equal(fs.readFileSync(testOut, {
			encoding: 'utf-8'
		}), fs.readFileSync(path.join(__dirname, 'resources/test.html.deduped.html'), {
			encoding: 'utf-8'
		}));
	});
});